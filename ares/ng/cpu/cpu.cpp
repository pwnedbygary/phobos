...
auto CPU::main() -> void {
  if(NeoGeo::Model::NeoGeoCD()) {
    ...
    if(f && n++ < 100000) fprintf(f, "%06x %04x\n", (u32)r.pc, (u32)r.irc);
    if(n == 100000 && f) { fflush(f); fclose(f); }
  }
  if(io.interruptPending) {
    ...
    //type1/type3. Priority order mirrors MAME: type3 > type2 > type1.
    if(NeoGeo::Model::NeoGeoCD()) {
      if(1 > r.i && lower(Interrupt::CDD)) {
        debugger.interrupt("CDD");
        if(cdd.type3Pending) { cdd.type3Pending = 0; return interrupt(Vector::CDDType3, 2); }
        if(cdd.type2Pending) { cdd.type2Pending = 0; return interrupt(Vector::CDDType2, 2); }
        if(cdd.type1Pending) { cdd.type1Pending = 0; return interrupt(Vector::CDDType1, 2); }
