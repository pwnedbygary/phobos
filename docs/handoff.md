...
**Neo Geo — Neo Geo CD (NGCD) M1 CORE PORT + ANDROID WIRING COMPILES (2026-08-24, UNCOMMITTED):** Staged port — M1 = BIOS-boot skeleton (BIOS → SNK logo via DMA upload into System RAM buffers `spriteRam`/`pcmRam`/`fixRam`), M2 = CD drive. AREs-side edits (`ares/ng/{ng.hpp,system/system.{hpp,cpp},cpu/{cpu.cpp,memory.cpp},apu/{apu.hpp,apu.cpp},lspc/render.cpp}`, new `mia/system/neo-geo-cd.cpp` + `mia/medium/neo-geo-cd.cpp` medium) plus Android wiring (`PhobosRunner.cpp` NGCD identification branch, `pak()` BIOS+zoomy block, load dispatch; `MainViewModel.kt` `fw_ng_cd` scan `neocd.zip`/`neocd.bin`/`ngcd.zip`/`ngcd.bin`; `FirmwareSettingsScreen.kt` `FirmwareInfo("Neo Geo CD",...,"fw_ng_cd")`) all COMPILE (`gradle assembleDebug` BUILD SUCCESSFUL). KEY GOTCHA: `Model` is BOTH `ares::NeoGeo::Model` (helper struct, `ng.hpp`) and `ares::NeoGeo::System::Model` (enum) — helper calls inside `System::` members MUST be `NeoGeo::Model::NeoGeoCD()` (unqualified `Model::` resolves to the enum and fails to compile). RUNTIME VERIFY PENDING: deploy + confirm SNK logo boots with a `neocd.zip` firmware present (`fw_ng_cd`). M2 (disc device, `disc.node`) not started.
