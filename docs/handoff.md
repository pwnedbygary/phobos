...
**Neo Geo compat pass (in progress 2026-08-20):** on-device results tracked in `docs/neo-geo-compatibility.md`. New findings today: **samsh5pf (SamSho V "Perfect" 2026 redump) boots to the SNK warning screen @59.2 FPS but is STUCK — START does not advance it (user-verified)**; K2K2 decrypt for this redump (`k2k2_sams5s`, key 0x0d) suspected wrong or the game waits in a protection/input loop — INVESTIGATION NEEDED. samsho + ssideki4 boot/gfx verified (see matrix). Palette-bank register inversion fix landed (commit `8746d3f56`). **ssideki4 in-match vertical/black sprite strips FIXED** — wrong LSPC `hscale` table in `ares/ng/lspc/lspc.cpp` (replaced with MAME `zoom_x_tables`) + added MVS horizontal wrap-around in `ares/ng/lspc/render.cpp`; user-verified clean pitch @~59.2 FPS.
