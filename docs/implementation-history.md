# Phobos implementation history archive

This is the archival companion to the [canonical implementation
plan](implementation-plan.md). It preserves the task names, task IDs, dated
findings, and disposition of the former 1,874-line plan without treating its
old queue, commands, or performance conclusions as current instructions. The
summaries below are a navigable index; the complete original text is appended
verbatim in the non-operative snapshot at the end of this file.

The snapshot was sourced locally with `git show HEAD:docs/implementation-plan.md`
from repository HEAD commit
`cd4a9ad05c7c94095bd1b987fb044ba7573a8150` (not a published or externally
resolvable revision). The source blob is
`7fb1891e29775f68d6a087fa2369ab30b7cc167f`, and the appended snapshot's
SHA-256 is
`52449993b2985e098b99ef9e1ff5ef4ccbd7266f6cd4b45dc3ccd26106d85a0a`.
The appendix is therefore the durable exact historical source, not an external
URL or an assertion that this local commit is available on GitHub.

## Archive rules

- Entries below are reported history, not fresh verification.
- “Verified” means the former plan reported that result; it does not mean this
  documentation pass reproduced it.
- Historical shell commands, device paths, commit recipes, and “next steps” are
  retained as evidence only. They are not authorization to run them.
- Current priorities and eligibility live only in
  [implementation-plan.md](implementation-plan.md), [performance-audit.md](performance-audit.md),
  and [development-process.md](development-process.md).

## Former plan headings and status index

The headings and IDs below intentionally retain the former plan's terminology
so old handoffs, task references, and review notes remain findable.

## Post-merge setup repair

Historical note: the former plan recorded the missing Replit hook repair and a
Gradle `help` validation as setup work, explicitly not an APK build or device
verification.

## 📌 Project Policies (user-stated)

The former plan recorded the accuracy/speed floor, documentation handoff rule,
commit/push habit, Compose `rememberUpdatedState` rule, controller HAT handling,
and race-free framebuffer guidance. The current process in
[development-process.md](development-process.md) supersedes the old commit,
push, broad-staging, and auto-deploy instructions.

## 🛠 Troubleshooting Playbook (read before debugging hangs / freezes)

The former ladder was: bounded instrumentation, counters/heartbeats, watchdog
and thread stacks, then a native debugger. It also recorded the N64
`vulkan.mutex` abandon-path invariant and the distinction between emulated
hardware reset and GPU-context reset. The ladder remains useful historical
diagnostic context; it is not a request for device work in this documentation
change.

## Status Summary

The former summary reported broad core coverage, N64 rendering fixes, N64DD
save/RTC work, PC Engine and ZX 128K fixes, and Neo Geo progress. It also
contained duplicated and superseded status text. The canonical plan retains
the dispositions and points to the dated records below rather than repeating
those snapshots as current validation.

## Priority Queue (OPEN items only — completed tasks archived in Detailed Task Notes)

The former queue asserted that completed tasks were removed from the queue.
That convention hid historical open work when later sessions changed the
queue. The canonical plan now inventories every old open item and gives each a
disposition.

## Detailed Task Notes (grouped by status — ordered by task number within each group)

#### <a id="task-9"></a>Task 9 — Ape Escape cinematics (FIXED & VERIFIED 2026-08-18, commit `021f13aa1`)

Historical evidence: the former plan attributed the white-screen/menu failure
to applying the PS1 XA file/channel filter to video sectors as well as audio
sectors. It reported filtering only XA audio sectors and on-device cinematic
verification.

#### <a id="n64-reset"></a>N64 reset (vulkan.mutex leak on reset) — FIXED 2026-08-11

Historical evidence: the former plan reported an unconditional scanout unmap,
pipeline-cache handling, soft-reset device retention, bounded waits, and stale
VI-fence clearing, followed by three reset recoveries.

#### <a id="task-5a"></a>Task 5a — N64 Vulkan experimental settings (DONE 2026-08-10)

Historical evidence: the former plan reported JNI/UI wiring for disabling VI
processing, supersample scanout, and weave deinterlacing.

#### <a id="task-5b"></a>Task 5b — Mario Tennis Adreno compute shader failures (RESOLVED 2026-08-10)

Historical evidence: the former plan reported Adreno compute-pipeline failures
and exposed diagnostic mitigations. It did not establish a performance result.

#### <a id="task-6"></a>Task 6 — Conker's BFD N64 freeze (RESOLVED 2026-08-12)

Historical evidence: the former plan reported a synchronous ares N64
`CPU::synchronize()` model and a `JitInterleaving` investigation ending at
`2048*2`, with larger cadence values associated with historical Conker stalls.
The current plan preserves the timing default and requires new evidence before
changing it.

#### <a id="task-20"></a>Task 20 — Audio crackling (FIXED 2026-08-09)

Historical evidence: the former plan attributed crackling to globally disabling
threaded video refresh and reported restoring threaded operation.

#### <a id="task-22"></a>Task 22 — GBC white screen (FIXED 2026-08-09)

Historical evidence: the former plan reported scheduler reset cleanup, threaded
operation, and the corrected boot-ROM offset.

#### <a id="task-23"></a>Task 23 — Mega Drive/Mega CD audio garbled (FIXED 2026-08-14)

Historical evidence: the former plan reported replacing one-stream-at-a-time
draining with a lockstep multi-stream mixer, mono duplication, clamping, and
registry cleanup. This is distinct from the cancelled Task #4 audio-buffer
reuse idea; Task #4 is not authorized or queued.

#### <a id="task-29"></a>Task 29 — PS1 analog toggle (FIXED 2026-08-09)

Historical evidence: the former plan reported identifying the Player 1 port by
name rather than a counter affected by the disc tray.

#### <a id="task-30"></a>Task 30 — Volume rocker (FIXED 2026-08-09)

Historical evidence: the former plan reported bypassing volume keys in the
Compose preview and key handlers.

#### <a id="task-32"></a>Task 32 — N64 C-button mappings: right-stick cardinal directions (DONE 2026-08-11)

Historical evidence: the former plan reported removing a stick-bit exclusion
from the hysteresis latch.

#### <a id="task-33"></a>Task 33 — ColecoVision BIOS (FIXED 2026-08-09)

Historical evidence: the former plan reported scanner keyword/CRC mappings and
the native `fw_coleco` lookup.

#### <a id="task-35"></a>Task 35 — Arcade removal (DONE 2026-08-09)

Historical evidence: the former plan reported removing the Arcade entry and
related JNI/native/UI paths while leaving the core tree.

#### <a id="task-38"></a>Task 38 — GBA RTC / Pokemon Unbound (FIXED 2026-08-13)

Historical evidence: the former plan reported broadened RTC detection, importing
saves before cartridge connection, pak resizing, and `.rtc` import/export.

#### <a id="task-39"></a>Task 39 — Wire Saves Path setting (DONE 2026-08-11)

Historical evidence: the former plan reported SAF path resolution and fallback
to internal saves.

#### <a id="task-40"></a>Task 40 — Vulkan Cache Path (DONE 2026-08-11)

Historical evidence: the former plan reported a configurable cache path,
copy-on-change, and internal fallback.

#### <a id="task-41"></a>Task 41 — States/Screenshots internal defaults (DONE 2026-08-11)

Historical evidence: the former plan reported SAF-or-internal fallback paths
for state files and screenshots.

#### <a id="task-45"></a>Task 45 — N64 performance pass (ESSENTIALLY FINISHED 2026-08-11)

Historical evidence: the former plan listed diagnostics removal, timing
tuning, CPU affinity, audio buffering, and several unmeasured candidates. The
current audit supersedes its optimization conclusions: no speedup is claimed
without matched native measurements.

#### <a id="task-47"></a>Task 47 — N64DD end-to-end support (DONE & VERIFIED 2026-08-11)

Historical evidence: the former plan reported firmware selection, secondary
medium loading, disk mounting, and F-Zero X Expansion Kit testing.

#### <a id="task-48"></a>Task 48 — N64 Rumble Pak / Controller Pak, pause-menu selectable (DONE)

Historical evidence: the former plan reported pak attachment/persistence,
rumble state and envelope handling, and user confirmation.

#### <a id="task-57"></a>Task 57 — PS1 DualShock analog toggle (FIXED 2026-08-13)

Historical evidence: the former plan reported cache invalidation and mutex
protection across recreated controller nodes, plus the corrected toggle return
value.

#### <a id="task-58"></a>Task 58 — dcache fast-path (REVERTED 2026-08-12)

Historical evidence: the former plan reported that bypassing dcache broke
Mario Tennis DMA visibility and was reverted. Task 59 is the parked successor;
the old fast path is not a candidate to revive blindly.

#### <a id="task-sg"></a>Task SG — SG-1000 verified (DONE 2026-08-14)

Historical evidence: the former plan reported SG-1000 playability and removal
of the SC-3000 app entry.

#### <a id="task-zx"></a>Task ZX — ZX Spectrum full support (DONE 2026-08-14)

Historical evidence: the former plan reported 48K/128K tape, keyboard,
controller, and signed-char fixes.

#### <a id="rogue-squadron"></a>Rogue Squadron (N64) main-menu garbage — RESOLVED 2026-08-15

Historical evidence: the former plan attributed the menu issue to the VI field
...
`_isLoaded` state during native teardown, and reported early state clearing,
native unload guarding, and zombie joining.

#### <a id="64dd-rtc"></a>64DD RTC seed (Error 48) — FIXED 2026-08-17

Historical evidence: the former plan reported seeding both zero- and
all-`0xff` RTC nodes and a device log showing the clock ticking.

#### <a id="gba-rtc-host"></a>GBA RTC host-time seed (Pokemon Unbound clock) — FIXED 2026-08-17

Historical evidence: the former plan reported clearing the halt state and
reseeding legacy epoch saves.

#### <a id="n64-save-persistence"></a>N64 save persistence (4-bug bundle) — ALL FIXED & VERIFIED 2026-08-17

Historical evidence: the former plan reported disk/error/RTC persistence,
medium ordering on reload, and cartridge save filters.

#### <a id="all-core-save-audit"></a>ALL-core save persistence audit — FILTER FIXES DONE 2026-08-17

Historical evidence: the former plan listed FC, GB/GBC, GBA, MD, N64, PS1,
SFC, and WS save coverage, while retaining sidecar, app-kill, and NGP export
verification as open sub-items.

#### <a id="stale-emu-thread"></a>Stale emu-thread handle on clean unload — FIXED 2026-08-17

Historical evidence: the former plan reported clearing clean-exit thread
handles while retaining them for abandon tracking.

#### <a id="task-17"></a>Task 17 — Auto-Save / Auto-Load State — DONE & VERIFIED 2026-08-17

Historical evidence: the former plan reported the reserved Auto slot,
save-before-teardown, load-before-thread-start, and pause-menu controls.

#### <a id="task-52"></a>Task 52 — PS1 multi-disc swap (FIXED & VERIFIED 2026-08-17)

Historical evidence: the former plan reported recursive Disc Tray lookup and
an MGS disc-2 hot-swap test.

#### <a id="task-62"></a>Task 62 — Dotprod/FP16/NEON optimization survey (DONE 2026-08-17)

Historical evidence: the former plan reported correct flavor flags and no
automatic dotprod/FP16 instructions in its surveyed build. The current audit
does not treat that survey as a universal code-generation or speed claim.

## Historical planned, parked, and in-flight headings

#### <a id="task-13c"></a>Task 13c — Multi-controller support (PLANNED 2026-08-11)

The former plan described per-port input state, stable device assignment,
player-specific JNI input, and per-port rumble. Tasks 13a, 13b, and 13d were
listed beside it in the QoL queue without separate detailed sections.

#### <a id="task-13a"></a>Task 13a — Per-core custom controller layouts (listed QoL work)

Historical queue entry retained; no separate detailed note existed in the
former plan. It remains grouped with the controller hierarchy in the canonical
plan.

#### <a id="task-13b"></a>Task 13b — Per-core controller rebinding (listed QoL work)

Historical queue entry retained; no separate detailed note existed in the
former plan.

#### <a id="task-13d"></a>Task 13d — Per-game controller rebinding (listed QoL work)

Historical queue entry retained; no separate detailed note existed in the
former plan.

#### <a id="task-14"></a>Task 14 — Responsive UI layout (listed QoL work)

Historical queue entry retained; no separate detailed note existed in the
former plan.

#### <a id="task-15"></a>Task 15 — QoL / polish / shader menu (listed QoL work)

Historical queue entry retained; no separate detailed note existed in the
former plan.

#### <a id="task-15a"></a>Task 15a — Quick-access pause menu items (listed QoL work)

Historical queue entry retained; no separate detailed note existed in the
former plan.

#### <a id="task-16"></a>Task 16 — Touch control resize / reposition (listed QoL work)

Historical queue entry retained; no separate detailed note existed in the
former plan.

#### <a id="task-18"></a>Task 18 — Video options / shaders (listed QoL work)
...
#### <a id="task-42"></a>Task 42 — Perf Monitor configurable metrics (PLANNED)
...
changes, with a content-diff caveat for single-buffered games.

#### <a id="task-43"></a>Task 43 — Perf Monitor GPU utilization (listed planned work)

Historical queue entry retained; no separate detailed note existed in the
former plan. Counter capability and attribution remain unknown.

#### <a id="task-44"></a>Task 44 — Perf Monitor CPU frequency and thermal metrics (listed planned work)

Historical queue entry retained; no separate detailed note existed in the
former plan.

#### <a id="tasks-43-44"></a>Tasks 43 & 44 — GPU/CPU/Thermal metrics (PLANNED)

The former plan proposed GPU counters/performance queries plus CPU-frequency
and thermal polling. No counters were measured in the current audit.

#### <a id="task-49"></a>Task 49 — N64 save import/export (PLANNED 2026-08-11)

The former plan proposed UI/file-copy support for `.sra`, `.eep`, `.fla`, and
`.mpk` mappings. It did not authorize changing save formats.

#### <a id="task-50"></a>Task 50 — Save-state screenshots (listed QoL work)

Historical queue entry retained; no separate detailed note existed in the
former plan.

#### <a id="task-51"></a>Task 51 — ZX multi-tape swap and multi-file ZIP picker (listed QoL work)

Historical queue entry retained; no separate detailed note existed in the
former plan.

#### <a id="task-59"></a>Task 59 — Proper write-through dcache bypass for cached RDRAM (PLANNED, PARKED)

The former plan proposed a per-game write-through design preserving SMC
tracking, after Task 58's DMA failure. It remains high-risk and parked.

#### <a id="task-60"></a>Task 60 — Per-game hash overrides table (PLANNED, PARKED)

The former plan proposed ROM-hash keyed timing/cache overrides and recorded
that its interleave experiment did not establish a need. It remains parked.

#### <a id="mischief-makers"></a>Mischief Makers (N64) title freeze — PARKED 2026-08-15

The former plan reported an interrupt-chain diagnosis and suspected upstream
RSP microcode accuracy, not the VU stubs. A reference-RSP comparison was
proposed, not completed.

#### <a id="task-ngcd-m2"></a>Task NGCD-M2 — RESOLVED (boot achieved 2026-08-27)

The former plan recorded the Neo Geo CD boot path: raw-sector lead-in/LBA
mapping, decoder IRQ vector `$15`, IPL-gated level-2 pending/ack handling,
controller-structure seeding, CD DMA, and the fixed one-times drive decision.
It reported SamSho RPG title/attract, BIOS menu, level-select, HUD, character,
and background rendering at roughly 59.2 FPS on the Retroid Pocket 6. Those
are historical reports, not measurements from this reconciliation.

#### <a id="neo-geo-cd-consolidated-reference"></a>Neo Geo CD consolidated reference (merged 2026-09-01)

The former detailed block preserved the swapped BIOS disassembly warning,
SamSho RPG test identity, `$C00108`/`$C0CA74` state flow, `$FF0061` DMA
trigger, `$FF000F` interrupt acknowledgements, CD settle bit-0 HLE, libretro
controller selector/active-low lanes, Geolith DMA references, MAME's
documentation-only register map, and the list of temporary diagnostics.
The canonical plan links here for the residual rather than repeating device
paths and commands.

#### <a id="task-ngcd-m3"></a>Task NGCD-M3 — NGCD rendering completeness: title-menu text residual (IN FLIGHT, static analysis 2026-09-01)

The former plan reported a SamSho RPG title-menu text overlap involving tile
families `0x2000`, `0x0c00`, and `0x4376`; global `+0x80` fetch and MAME's
odd-byte model were ruled out. Its static comparison identified a possible
`0xfc2d` word-destination phase difference against Geolith, while explicitly
requiring an instrumented device check. This remains an open compatibility
investigation, not permission to change DMA.

<a id="task-10a"></a>

#### <a id="task-10c"></a>Task 10c — PCE family / ZX 128K / Neo Geo MVS-AES status

The former priority queue labelled this combined scope **Task 10c/10a**;
the original detailed heading is retained verbatim here.

The former plan reported PCE/SuperGrafx and ZX 128K fixes, then Neo Geo
warning-screen, PROGSF1, SMA, coin, and switch-game fixes. The Neo Geo core
was later described as broadly working, with compatibility coverage and
title/gameplay residuals still needing disposition. The canonical plan treats
the core fixes as historical complete work and retains compatibility as an
open, bounded matrix task.

<a id="swap-screen"></a>

## Feature Log — Swap-Screen Hotkey (2026-08-18, VERIFIED on-device)

The former plan recorded pause-on-leave, return navigation, window-callback
key interception, debug-loader navigation, and the explicit quit-to-unload
decision. It also recorded that held hotkey combinations cannot be tested with
single `adb input keyevent` calls.

#### <a id="feature-completeness"></a>Feature-completeness audit: Run-Ahead + Fast Boot + Skip Boot ROM (INVESTIGATED 2026-08-17)

The former audit found Run-Ahead's UI toggle unwired, Fast Boot supported only
on selected cores, and Skip Boot ROM supported only on GB/GBC/WS. It reported
Fast Boot being moved to per-core Boot Options. The canonical plan keeps
Run-Ahead open and Fast Boot/Skip Boot resolved as scope-limited.

#### <a id="task-46"></a>Task 46 — PS1 CD-DA audio pops (FIXED & VERIFIED 2026-08-18)

Historical evidence: the former plan reported a short fade envelope around
CD-DA playback-state transitions and healthy audio-buffer observations.

#### <a id="task-61"></a>Task 61 — Proper release APK signing (Play-ready / In-place upgrades) (FIXED & VERIFIED 2026-08-20)

Historical evidence: the former plan reported a stable release keystore and
in-place upgrade verification. Current release/update safety is separately
tracked by Task #2 and is blocked pending input; this archive does not turn the
old report into a current signing assertion.

#### <a id="task-63"></a>Task 63 — PS1 CPU (R3000) recompiler (OPEN — the big one)

The former plan proposed adapting the N64 SLJIT recompiler with an interpreter
fallback. No speed result was established.

#### <a id="task-64"></a>Task 64 — PS1 GPU off-CPU (OPEN)

The former plan proposed a low-risk blitter experiment and a much larger
Vulkan renderer port. Neither is authorized by the current evidence baseline.

#### <a id="task-65"></a>Task 65 — SNES CPU (WDC65816) recompiler (OPEN)

The former plan proposed an SLJIT recompiler for the interpreter-only CPU.

#### <a id="task-66"></a>Task 66 — GBA CPU (ARM7TDmi) recompiler (OPEN)

The former plan proposed a lower-priority SLJIT recompiler for demanding GBA
titles.

#### <a id="task-67"></a>Task 67 — Genesis (MD) performance VDP (OPEN)

The former plan proposed testing the performance VDP behind a flag and warned
about duplicate VDP wiring. It did not establish a benefit.

#### <a id="task-68"></a>Task 68 — N64 RSP VU ARM64 NEON verification (OPEN)

The former plan stated that `sse2neon.h` translated SSE intrinsics to NEON
and proposed verifying generated code. Its stronger “native NEON” comparison
was not measured and is corrected in the canonical plan.

#### <a id="task-69"></a>Task 69 — Performance overlay CPU-vs-GPU breakdown (OPEN)

The former plan proposed a CPU/GPU metric to make later experiments
attributable. The current audit first requires low-overhead external traces.

#### <a id="task-70"></a>Task 70 — ZX Spectrum (z80) recompiler (OPEN — LOW / probably skip)

The former plan correctly advised profiling before touching this low-demand
core. It remains deferred.

#### <a id="task-71"></a>Task 71 — Dynamic speed compensation (OPEN, renumbered from 52)

The former plan proposed scaling emulated clock/audio to smooth low-FPS
rendering. It remains a product/accuracy decision, not a performance fix.

#### <a id="task-72"></a>Task 72 — N64 RDP-ParaLLEl performance investigation vs mupen64plus-ae-turnip (OPEN)

The former plan recorded a code comparison that attributed a reported
difference to async RDP, RSP SIMD, core philosophy, overclock granularity,
frame pacing, and presentation. The current audit keeps the comparison as a
candidate investigation but withdraws its magnitude and transplant
recommendations until matched evidence exists.

#### 2026-08-19 → 2026-08-20 — GPU Driver Downloader (QoL, DONE) + Neo Geo input/BIOS fixes (UNCOMMITTED)

Historical evidence: the former plan recorded driver selection/download/delete,
truthful Neo Geo BIOS/ROM errors, HAT D-pad mapping, and tall-sprite/palette
fixes. “Uncommitted” was the state reported by that dated note; it is not a
current worktree assertion.

#### 2026-08-22 → 2026-08-23 — Neo Geo: SMA decrypt endianness + BML word_swap FIXED & VERIFIED ON-DEVICE

Historical evidence: the former plan reported removing a hardcoded SMA reset
vector, correcting big-endian decryption, improving SELECT/coin polling, and
removing unsafe debug reads. Audio and `ssideki4` notes in that section were
left as historical follow-up questions.

## NEO GEO CD (future task — reference)

The former plan pointed to the ares `neogeo-cd` branch as a future reference
and instructed a selective CD-ROM/CD-audio port. That future note is superseded
by the later NGCD-M2/M3 records above; it remains here so the original heading
and decision trail are not lost.

## Historical performance section (superseded)
...
The former plan's historical analysis under these headings asserted “2–3×” and
“massive” async-RDP gains, treated the RSP comparison as native-NEON versus
SSE-to-NEON, and described presentation differences as explanatory. The
current [performance audit](performance-audit.md) corrects all three:

1. SyncFull waits are observable source differences, not a safe bypass. RDRAM
   visibility, DP interrupt ordering, readback, reset, save/load, unload, and
   timeout ownership must be audited first.
2. `sse2neon`/the ARM64 SIMD gate does not prove scalar fallback, native-NEON
   superiority, or a speed result. Exact release objects and workload samples
   are required.
3. The presentation paths contain maps/copies/locks on both sides; no
   measured attribution or faster/simpler-path conclusion exists.

The old performance references are retained in the source revision and this
disposition, but are not active implementation instructions.

## Other historical policy headings

The former plan also contained:

- `### QoL-phase tasks (after core stability)` — Tasks 13a, 13b, 13c, 13d,
  14, 15, 15a, 16, 18, 19, 31, 42, 42b, 43, 44, 50, 51, and 71.
- `### Performance sub-batch` — Tasks 63, 64, 65, 66, 67, 68, 69, and 70.
- `### FUTURE — Neo Geo CD core (post-1.0, additive)` — superseded by the
  later NGCD-M2/M3 history above.
- `### OUT OF SCOPE — Sega Saturn` — still out of scope pending an explicit
  product and licensing decision.


## Complete original plan snapshot — non-operative historical appendix

This fenced appendix is an exact local snapshot of
`git show HEAD:docs/implementation-plan.md`. It is historical reference only;
the compact roadmap above is authoritative. The source is the local repository
HEAD commit `cd4a9ad05c7c94095bd1b987fb044ba7573a8150`; that commit is not claimed
as a published or externally resolvable revision. The source blob is
`7fb1891e29775f68d6a087fa2369ab30b7cc167f`, and the exact source snapshot
SHA-256 is `52449993b2985e098b99ef9e1ff5ef4ccbd7266f6cd4b45dc3ccd26106d85a0a`.
...
>   and **CD-speed cap at 2×**: the CDD-tick multiplier looked safe in theory (one sector
>   + one decoder IRQ per tick at any N), but at >2× the BIOS's access machine
>   ($C0E99E) cannot complete a state transition before the next sector IRQ arrives →
>   next header read returns MSF=0 / no-response → **DISC I/O ERROR ID=0000 or 0002**.
>   The 68K's instruction rate also visibly drops at high N (the CDD tick burns more
>   CPU per 68K instruction), so the boot intro slows and tears. The Java dropdown now
>   only offers 1×..2×, and `PhobosRunner.cpp::setCdSpeed` caps any persisted value >2
>   (logged) so older DataStore entries can't push the device into the broken state.
>   The ring-buffer safety net in `tick()` remains as the rare-transient guard.
