...
# 1. Reset modified files in ares, nall, and mia back to clean git state
...
# 9. Generate CMakeLists.txt WITH nall/nall AS ABSOLUTE HIGHEST INCLUDE PRIORITY
include_dirs = [
    "${CMAKE_CURRENT_SOURCE_DIR}/nall/nall",  # 1. Matches <resource/resource.hpp> directly to nall/nall/resource/resource.hpp
...
print("✏️ Wrote master CMakeLists.txt with nall/nall high-priority include path.")
...
subprocess.run(["git", "commit", "-m", "fix: Add nall/nall to CMake include paths to resolve resource.hpp collision"], capture_output=True)
