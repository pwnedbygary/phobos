...
# 1. Collect all extra header include directories, strictly excluding any Vulkan-Headers desktop folder (case-insensitive)
extra_includes = set()
...
for search_base in ["thirdparty", os.path.join("ares", "n64", "vulkan")]:
    if os.path.exists(search_base):
        for root, dirs, files in os.walk(search_base):
            # Case-insensitive exclusion of desktop Vulkan-Headers
...
print("✏️ Updated CMakeLists.txt (excluded desktop Vulkan-Headers).")
...
os.system("git add .")
os.system('git commit -m "fix: Exclude desktop Vulkan-Headers case-insensitively to use Android NDK Vulkan"')
